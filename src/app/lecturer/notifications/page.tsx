...
import { MarkAllRead } from '@/app/portal/notifications/mark-all-read';
...
export default async function LecturerNotificationsPage() {
  ...
  return (
    <>
      <PageHeader
        title="Notifications"
        description="Submissions, messages and university announcements."
            ...
            {notifications.map((notification) => (
              ...
              >
                <div className="min-w-0">
                  ...
                  {notification.link ? (
                    <Link href={notification.link} className="mt-1.5 inline-block text-sm font-medium text-brand hover:underline">
                      Open →
                    </Link>
                  ) : null}
                </div>
                <span className="shrink-0 text-xs text-muted">{formatRelative(notification.createdAt)}</span>
              </div>
            ))}
          </CardBody>
        </Card>
